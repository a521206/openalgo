...
def analyze_position_change(
    current_position_qty: int, action: str, quantity: int
) -> PositionChange:
    """
...
def get_lot_size_with_status(symbol: str, exchange: str) -> LotSizeResult:
    ...
    if exchange in ["NSE", "BSE"]:
        return LotSizeResult(
            lot_size=1, status=LotSizeFetchStatus.EQUITY_DEFAULT, exchange=exchange
        )
...
def validate_against_smart_trade_rules(
    ...
    position_based_rules_enabled = (
        rules.get("prevent_duplicate_buy", False)
        or rules.get("prevent_duplicate_sell", False)
        or rules.get("max_position_size") is not None
    )

    if position_result.status == PositionFetchStatus.FAILED and position_based_rules_enabled:
        return (
            False,
            f"Smart Trade Rule: Cannot validate order - position fetch failed: {position_result.error_message}",
        )
    ...
    product_type = (order_data.get("product") or order_data.get("product_type") or "MIS").upper()
    ...
    if rules.get("prevent_duplicate_buy", False):
        if action == "BUY" and current_position_qty > 0:
            return (
                False,
                f"Smart Trade Rule: Duplicate BUY blocked. You already have a long position of {current_position_qty} in {symbol}",
            )
    ...
    if rules.get("prevent_duplicate_sell", False):
        if action == "SELL" and current_position_qty < 0:
            return (
                False,
                f"Smart Trade Rule: Duplicate SELL blocked. Already have a short position of {abs(current_position_qty)} in {symbol}",
            )
    ...
    if max_position_size is not None and max_position_size > 0:
        ...
        if position_change.change_type in [
            PositionChangeType.INCREASING,
            PositionChangeType.OPENING,
            PositionChangeType.REVERSING,
        ]:
    ...
    if max_order_value is not None and max_order_value > 0:
        ...
        if order_value > max_order_value:
            return (
                False,
                f"Smart Trade Rule: Maximum order value exceeded. Limit: ₹{max_order_value:,.2f}, Order value: ₹{order_value:,.2f}",
            )
    ...
    if rules.get("allow_intraday_only", False):
        if product_type not in ["MIS", "INTRADAY"]:
            return (
                False,
                f"Smart Trade Rule: Only intraday (MIS) orders allowed. Product type '{product_type}' is blocked",
            )
