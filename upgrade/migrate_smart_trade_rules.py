#!/usr/bin/env python3
"""
Migration script to add Smart Trade Rules columns to existing settings table.
This resolves the "no such column: settings.smart_trade_enabled" error.

Usage:
    cd upgrade
    uv run migrate_smart_trade_rules.py
"""
...
def migrate_settings_table():
    """Add missing Smart Trade Rules columns to the settings table if they don't exist"""
    ...
    try:
        ...
        # Define the Smart Trade Rules columns that should exist
        smart_trade_columns = [
            ("smart_trade_enabled", "BOOLEAN DEFAULT 1"),
            ("prevent_duplicate_buy", "BOOLEAN DEFAULT 1"),
            ("prevent_duplicate_sell", "BOOLEAN DEFAULT 1"),
        ...
        with engine.connect() as conn:
            for column_name, column_def in smart_trade_columns:
        ...
        logger.info(f"   - Total smart trade columns: {len(smart_trade_columns)}")

        if columns_added > 0:
            logger.info("\n✅ Settings table migration completed successfully!")
            logger.info("   Smart Trade Rules columns have been added to your database.")
        else:
            logger.info("\n✅ No migration needed - all Smart Trade Rules columns already exist!")
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("OpenAlgo Smart Trade Rules Migration Script")
    logger.info("=" * 60)
    logger.info("This script adds missing Smart Trade Rules columns to the settings table")
    logger.info("-" * 60)
    ...
    if success:
        ...
        logger.info("   2. Navigate to /admin/smart-trade-rules to configure rules")
        return 0
    else:
